...
def pref(operandbytes=sbytes("NONE"), pickleable=sbytes("NONE")):
    ...
    headalen = intbitter(ver+fieldbyte+1+codalen)
    # 4+1+1+6
...
def pdref(operandbytes):
    ...
    def bytewise_NOTORNOT(byties,bytiestwo):
        ...
        zip = list(zip(byties,bytiestwo))
        zippzer = [ ~(z[0]|~z[1]) for z in zop ]
        return functools.reduce(lambda x,y: x+y, zippzer)
    ...
    if fieldbytes.decode("utf8").startswith(tuple(ke.decode("utf8") for ke in byte2field.keys())):   
        err.append(("IPCPROTO_ERROR",f"YOU DECODED SOMETHING SOMETHING SUSPICIOUSLY LIKE A NONE-FIELD:{repr(fieldbytes)}"))
        fieldbytes = b"NNNnnn"
    field = byte2field[fieldbytes]
    headalen = operandbytes[5:6] #teehee
    #codalen = operandbytes[6:headalen]
    coda = pickle.loads(operandbytes[headalen:])
